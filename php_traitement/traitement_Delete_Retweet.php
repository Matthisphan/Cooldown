...
if($isAuth){          
    if(array_key_exists('id_tweet', $json) && $json['id_tweet'] != null){
        ...
        if(sizeof($retweetResult) > 0){
            $sql = 'DELETE FROM retweet WHERE id_tweet = :id_tweet AND id_users = :id_users';
            $deleteRetweet = $db->prepare($sql);
            $deleteRetweet->execute($sqlParams) or die($db->errorInfo());
            echo "retweet supprimé";
        } else {
            echo "Vous n'avez pas encore retweeté ce tweet";
        }
    } else {
