...
if ($isAuth) {
    if (array_key_exists('id_tweet', $json) && $json['id_tweet'] != null) {
        ...
        if (sizeof($likeResult) > 0) {
            echo "Vous avez déjà aimé ce tweet";
        } else {
            $sql = 'INSERT INTO likes(id_tweet, id_users) VALUES (:id_tweet, :id_users)';
            $insertLike = $db->prepare($sql);
            $insertLike->execute($sqlParams) or die($db->errorInfo());
        }
    }
}
?>
